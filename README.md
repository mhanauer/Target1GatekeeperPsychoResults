...
fit3Try4 = cfa(model3Try4, estimator = "MLR",  missing = "ML", data = Sec3PsychCFA[c(-1)])
summary(fit3Try4, fit.measures = TRUE, standardized = TRUE)

modificationindices(fit3Try2)

anova(fit3Try2, fit3Try4, fit3)
...
describe(Sec2IRT)
fitOrdGRM_Measure2 = grm(data = Sec2IRT, constrained = FALSE)
...
```
Get data ready need to get the demographics and include time
```{r}
...
genderM2_DIF = lordif(resp.data = datIRT_M2Items, group = datIRT_M2Descript$Gender, criterion = "Chisqr", alpha = .01, minCell = 5)
summary(genderM2_DIF)
plot(genderM2_DIF)


raceM2_DIF = lordif(resp.data = datIRT_M2Items, group = datIRT_M2Descript$Race, criterion = "Chisqr", alpha = .01, minCell = 5)
summary(raceM2_DIF)
plot(raceM2_DIF)
...
```
Now diff for measure three (assuming a second order sturcture so it is unidimensional)
```{r}
datIRT_M3 = datPrePost3monthSec3Psych
datIRT_M3Descript = datIRT_M3[c(1:5)]

datIRT_M3Items = datIRT_M3[c(6:13)]
